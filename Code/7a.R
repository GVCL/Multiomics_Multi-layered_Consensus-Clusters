...
load('dims3c.Rdata')
L1_mat = as.matrix(read.csv(BiCorrFiltered_fname, sep=",", header = TRUE))
dim(L1_mat)
head(L1_mat)
...
load('dims6a.Rdata')
meanthreshold = L1_meanthreshold
...
for (i in 1:len){   
  ...
  if (nrow(ClustersNew) > meanthreshold)
  {
    ...
    {  
      ...
      newL1 <- L1_mat[(match(methygenes,colnames(L1_mat))),]
      L1 <- newL1[,(match(methygenes, colnames(L1_mat)))]
      
      g1 <- graph_from_adjacency_matrix(abs(L1), weighted=TRUE, mode="undirected")
      ...
      Degree <-  paste("methy_",selectedDegree, sep="")
      Between <- paste("methy_",selectedBetween, sep="")
      Ececen <- paste("methy_",selectedEccen, sep="")
...
write.table(finalDegreeGenes, paste0("Results/SelectedGenes/L1/NodeDegreeBasedGenes_", val_str, ".csv"), sep=",", col.names = c("Genes"))
write.table(finalBetweenGenes, paste0("Results/SelectedGenes/L1/NodeBetweenessBasedGenes_", val_str, ".csv"), sep=",", col.names = c("Genes"))
write.table(finalEccenGenes, paste0("Results/SelectedGenes/L1/NodeEccentricityBasedGenes_", val_str, ".csv"), sep=",", col.names = c("Genes"))
write.table(SelectedHCCNodes, paste0("Results/SelectedGenes/L1/HCCs-FinalGenes_", val_str, ".csv"), sep=",", col.names = c("Genes"))
...
newL1 <- L1_mat[(match(mRNAgenes,colnames(L1_mat))),]
L1 <- newL1[,(match(mRNAgenes, colnames(L1_mat)))]
g2 <- graph_from_adjacency_matrix(abs(L1), weighted=TRUE, mode="undirected")
...
write.table(finalDegreeGenes, paste0("Results/SelectedGenes/L1/NCCs-NodeDegreeBasedGenes_", val_str, ".csv"), sep=",", col.names = c("Genes"))
write.table(finalBetweenGenes, paste0("Results/SelectedGenes/L1/NCCs-NodeBetweenessBasedGenes_", val_str, ".csv"), sep=",", col.names = c("Genes"))
write.table(finalEccenGenes, paste0("Results/SelectedGenes/L1/NCCs-NodeEccentricityBasedGenes_", val_str, ".csv"), sep=",", col.names = c("Genes"))
write.table(SelectedNCCNodes, paste0("Results/SelectedGenes/L1/NCCs-FinalGenes_", val_str, ".csv"), sep=",", col.names = c("Genes"))
