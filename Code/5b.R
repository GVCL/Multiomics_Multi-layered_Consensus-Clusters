...
fileread <- data.table(read.csv("Results/Communities/L2/parCor-igraphCommunities-pvalueBased.csv", sep=",", header = TRUE))
head(fileread)
k <- nrow(fileread)
...
write.table(commonMat, "Results/Communities/L2/Common-Matrix.csv", sep=",", row.names = FALSE)
write.table(likelihoodMat, "Results/Communities/L2/CoANs-Matrix.csv", sep=",", row.names = FALSE)
write.table(df, "Results/Communities/L2/CoANs-Edges.csv", sep=",", col.names= c("Source","Target","CoONs-Averaged-EdgeWeight"), row.names = FALSE)

dim(commonMat)
nnzero(commonMat)

list <- as.matrix(read.csv("Results/Communities/L2/Common-Matrix.csv", sep=",", header = TRUE))
...
write.table(x, "Results/Communities/L2/L2_CoANsCommunities_Louvain.csv", sep=",", col.names= c("Nodes","NodeNames","Communities"), row.names = FALSE)
